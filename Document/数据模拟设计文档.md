...
> **版本**: V1.5.0 · 2026-03-27
> **状态**: 已实现 (No-Redis 架构优化版)
...
11. **V1.5.0 新增**：**No-Redis 架构优化**。为降低本地开发与小规模部署难度，移除了 Redis 进程间通信逻辑，统一使用 `SystemLog` 数据库表作为运行状态的实时间日志中介。
---
...
while True:
    ...
    # ── 提交进度日志 (V1.5.0 DB 模式) ──────────
    message = f"ENGINE: {records_created} nodes written successfully."
    SystemLog.objects.create(message=message, log_type='engine_log', timestamp=now)
    # 清理旧日志（保持 100 条）
    if tick % 20 == 0:
...
### 5.4 No-Redis 本地开发架构优化 (V1.5.0)

在 V1.5.0 及以后的版本中，为了降低本地部署难度并提升系统健壮性，我们对进程间通信（IPC）进行了如下调整：

- **移除 Redis 依赖**：去处了原本用于日志广播的 Redis List 队列，系统不再寻找本地 6379 端口。
- **SystemLog 中介模式**：仿真引擎直接向 `SystemLog` 模型写入心跳日志。该模型自带 `timestamp` 和 `log_type` 字段，支持分页和批量清理。
- **前端适配**：前端页面由原来的“WebSocket/Redis 监听”变更为“通过后端 API 读取 SystemLog 快照”，实现了在无 Redis 环境下依然可以看到监控引擎实时心跳的效果。
...
> **时效性说明 (V2.2.0 架构升级)**：
> 为了消除逻辑碎片并确保数据一致性，系统将所有重置逻辑下沉至模型层。
>
> **统一接口位置**：`monitor/models.py` -> `DeviceInfo.initial_repair_all()`
>
> **实现方法**：
> - 封装了 **5-6-7-5-2** 的标准风险梯度分布逻辑。
> - 强制清空 `maintenance_advice` 指令，恢复至默认平稳描述。
> - 使用 `transaction.atomic` 和 `bulk_update` 保证数据变更的原子性与高性能。
>
> **调用链 (三位一体同步)**：
> 1. **服务器重启**：由 `monitor/apps.py` 触发自检。
> 2. **手动全局重置**：点击 [立即重置] 按钮，通过 `views.py` 或 `consumers.py` 调用。
> 3. **仿真脚本启动**：`simulate_real_cnc_data.py` 在初始化阶段同步清理。
