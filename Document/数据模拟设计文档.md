...
>
> **[V2.2.0 联动重置] 维修建议同步清空机制**：
> 为了保证演示的一致性，系统建立了“三位一体”的建议清洗机制：
> 1. **服务器重启 (`apps.py`)**：启动自检时，若触发分组修复，同步将所有设备的 `maintenance_advice` 重置为“平稳”状态。
> 2. **全局点击 (`consumers.py`)**：前端点击 [立即重置] 时，在原子事务中批量清空所有人工录入的工单指令。
> 3. **仿真启动 (`simulate_real_cnc_data.py`)**：实时引擎初始心跳前，全量覆写设备指令字段。
