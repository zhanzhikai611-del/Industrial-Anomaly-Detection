...
| 组别 | 设备范围 | 设备名义状态 | `sc_mean` | `sp_mean` | AI 风险概率目标 | 风险等级 |
|------|---------|------------|-----------|-----------|---------------|---------|
| Group 0 | **[1-5]** (idx 0-4) | 新设备 | 15.0 | 0.13 | 0%—40% | 🟢 低风险 |
| Group 1 | **[6-10]** (idx 5-9) | 准新设备 | 24.0 | 0.18 | 30%—50% | 🟢🟡 低~中风险 |
| Group 2 | **[11-15]** (idx 10-14) | 正常设备 | 27.0 | 0.25 | 41%—75% | 🟡 中风险 |
| Group 3 | **[16-20]** (idx 15-19) | 老旧设备 | 31.0 | 0.32 | 60%—85% | 🟡🔴 中~高风险 |
| Group 4 | **[21-25]** (idx 20-24) | 故障边缘 | 40.0 | 0.40 | 76%—100% | 🔴 高风险 |

> **原理**：逻辑回归模型的决策函数为 sigmoid(w·x + b)。通过偏移输入特征的均值（`sc_mean`, `sp_mean`），改变了 w·x 的数值，从而使 sigmoid 输出（即预测概率）落入不同区间。

```python
GROUP_PARAMS = [
    (15.0, 0.13),  # Group 0: New       → ~0-40%
    (24.0, 0.18),  # Group 1: Semi-new  → ~30-50%
    (27.0, 0.25),  # Group 2: Normal    → ~41-75%
    (31.0, 0.32),  # Group 3: Old       → ~60-85%
    (40.0, 0.40),  # Group 4: Faulty    → ~76-100%
]
```

#### 4.3.2 分组索引映射

设备按 0-indexed 的创建顺序映射到组别：

```python
for idx, dev in enumerate(devices):
    if idx < 5:       group_idx = 0   # 设备 1-5
    elif idx < 10:    group_idx = 1   # 设备 6-10
    elif idx < 15:    group_idx = 2   # 设备 11-15
    elif idx < 20:    group_idx = 3   # 设备 16-20
    else:             group_idx = 4   # 设备 21-25
```
